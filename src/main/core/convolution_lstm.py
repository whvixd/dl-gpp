...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, kernel_size, step=1, effective_step=[1]):
        ...
        self._all_layers = []
        # ConvLstm隐藏层层数，hidden_channel 隐藏层大小大小
        for i in range(self.num_layers):
            name = 'cell{}'.format(i)

            # 对于同一个时刻，Cell中多层结构和BP是一样的,如：
            # 多变量Xt(3X3),输入层:x1,x2,x3 -ht-> 隐藏层1:h1,h2,h3,h4,h5 -ht-> 隐藏层2:h1,h2,h3,h4,h5 -FC-> Yt(1X1)
    ...
    def forward(self, input):
        ...
        for step in range(self.step):
            # x = input#原作者，入参数据缺了time seq 步长，和cell中的bias
            x = input[step]
            for i in range(self.num_layers):
...
if __name__ == '__main__':
    ...
    input = Variable(torch.randn(5,1, 512, 64, 32))
