...
def baseline_train(olddata, f, trees):
  ...
  if(os.path.isfile(saving_model)):
    print(
      "Loading in a pre-made, large read-only data structure we previously made with training data to use for approximate nearest neighbors on holdout data...")
    t.load(saving_model)
  else: 
    print(
      "Creating a large read-only data structure with training data to use for approximate nearest neighbors on holdout data...")
